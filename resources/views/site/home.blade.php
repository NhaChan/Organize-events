...
@php
    ...
    if ($heroEvent) {
        ...
        $heroFit = $heroEvent->thumbnail_fit ?: 'cover';
        $heroPosition = $heroEvent->thumbnail_position_y ?? 50;
        ...
        $heroFit = $heroCategory->page->service_image_fit ?: 'cover';
        $heroPosition = $heroCategory->page->service_image_position_y ?? 50;
        ...
        $heroFit = $heroCategory->page->banner_image_fit ?: 'cover';
        $heroPosition = $heroCategory->page->banner_image_position_y ?? 50;
    } else {
        $heroImage = null;
        ...
        @forelse($serviceCategories as $i => $cat)
            @php
                ...
                if ($cat->page?->service_image) {
                    ...
                    $serviceCaption = $cat->page->service_image_caption;
                    $serviceFit = $cat->page->service_image_fit ?: 'cover';
                    $servicePosition = $cat->page->service_image_position_y ?? 50;
                    ...
                    $serviceCaption = $cat->page->banner_caption;
                    $serviceFit = $cat->page->banner_image_fit ?: 'cover';
                    $servicePosition = $cat->page->banner_image_position_y ?? 50;
                }
            @endphp
...
<section class="party-hero home-hero">
    ...
    <div class="hero-showcase">
        @if($heroImage)<img src="{{ $heroImage }}" alt="{{ $heroAlt }}" loading="lazy" fetchpriority="low" decoding="async" style="object-fit:{{ $heroFit }};object-position:center {{ $heroPosition }}%">@else<div class="hero-image-fallback">🎈</div>@endif
...
<section class="party-section posts-section"><div class="section-heading left"><span>HÌNH ẢNH & BÀI VIẾT</span><h2>Các sự kiện và mẫu mới nhất</h2></div><div class="post-grid">@forelse($latest->take(8) as $event)<x-event-card :event="$event"/>@empty<div class="empty-state">Chưa có bài viết. Admin hãy đăng bài đầu tiên để nội dung xuất hiện ở đây.</div>@endforelse</div><div class="center"><a class="party-btn outline" href="{{ route('events') }}">Xem tất cả bài viết</a></div></section>

<section class="contact-banner"><div><span class="text-red-500">BẠN ĐANG CHUẨN BỊ MỘT NGÀY VUI?</span><h2>Liên hệ để được tư vấn dịch vụ phù hợp</h2><p>Trao đổi trực tiếp qua điện thoại, Facebook hoặc fanpage để chọn dịch vụ phù hợp.</p></div><div class="contact-actions"><a href="tel:{{ preg_replace('/\s+/', '', $settings['phone']) }}">☎ {{ $settings['phone'] }}</a>@if($settings['facebook'])<a href="{{ $settings['facebook'] }}" target="_blank" rel="noopener">Facebook</a>@endif @if($settings['fanpage'])<a href="{{ $settings['fanpage'] }}" target="_blank" rel="noopener">Fanpage</a>@endif</div></section>
@endsection
